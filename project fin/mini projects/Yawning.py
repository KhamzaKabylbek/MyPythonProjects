...
predictor = dlib.shape_predictor('Face Recognition/shape_predictor_68_face_landmarks.dat')

cap = cv2.VideoCapture(1)

yawn_count = 0  # Initialize yawning counter
...
while True:
    ...
    for face in faces:
        ...
        if mouth_ratio < 1.65:
            yawn_count += 1  # Increment yawning counter
            cv2.putText(img, "Yawning", (20, 50), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 255, 0), 2)

    # Display yawning count
    cv2.putText(img, f"Yawn Count: {yawn_count}", (20, 100), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 255, 0), 2)
